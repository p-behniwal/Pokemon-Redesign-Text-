...
Prabhgun Behniwal
Pokemon Project
Text-version game of Pokemon where a team of pokemon battle another team of pokemon. Since there is a large amount of pokemon, I chose to battle just a few of the pokemon rather than every single pokemon
 */
...
public class PokemonArena {
    ...
    private Random randNum = new Random();  //To generate random numbers easier
    ...
    public PokemonArena(ArrayList<Integer> selectedPokemons, int enemycount) {  //Constructor
        ...
        for (int p : selectedPokemons) {  //Iterates through all of the player's Pokemons
            playerPokemons.add(new AttackPokemon(pokedex.getPokemon(p)));  //Makes attackpokemon objects for battling
        }

        for (Map.Entry<Integer, Pokemon> entry : pokedex.getPokeDex().entrySet()) {  //Iterates through the rest of the pokemons
            if (!selectedPokemons.contains(entry.getKey())) {  //Sees if the user selected this pokemon
                unselected_pokemon.add(entry.getKey()); //Adds to unselected_pokemon if the pokemon has not been used
            }
        }

        for (int i = 0; i < enemycount; i++) {  //Chooses the pokemon for the enemy
            computer_selected = randNum.nextInt(unselected_pokemon.size());  //Selects a pokemon using a random number generator

            enemyPokemon.add(unselected_pokemon.get(computer_selected));  //Adds the pokemon selected to the enemy's pokemons
            unselected_pokemon.remove(computer_selected);  //Removes from the unselected pokemons
        }
    }
    ...
    public boolean selectPokemon() {
        ...
        pokeTools.clearScreen();  //Clears the screen and printing out the column headings
...
//        pokeTools.delayPrintTable("╠══════════════════════════════════╬════════════════════════════╬═══════╬═══════╬══════════════════════╬══════════════════════╬══════════════════════╬════════════╬════════════╣");
        ...
        for (AttackPokemon p : playerPokemons) {  //Iterates through all of the player's pokemons to be selected
            if (p.isAlive()) {  //If the pokemon is alive
                counter++;  //Increases the counter so that each pokemon has a different number
        ...
        pokeTools.clearScreen();  //Clears the screen
        pokeTools.delayPrintln(name + ": " + pokemonSelected.getName() + ", I choose you");  //Prints the I choose you message
    ...
    public void playerAction() {
        ...
        if (pokemonSelected.isStunned()) {  //If stunned then the pokemon skips the turn
            ...
            while (true) {  //While the user has not selected a move
                ...
                option = pokeTools.getRange("Please select a valid option", "Select your Action >>> ", 1, 3); //Gets input
                if (option == 1 && pokemonSelected.playerAttack(enemySelected, name)) {  //Checks if the option is 1 and if the player actually attacked
    ...
    public void roundEnd() {  //Runs after the round ends, restores 20EC for all player pokemons and enemy active pokemon
        ...
        pokemonSelected.roundEnd();  // Adding fEC to all the active pokemons
    ...
    public void startBattle() {
        ...
        while (enemyPokemon.size() != 0) {  //Checking if a winner can be determined
            cPokemon = randNum.nextInt(enemyPokemon.size());  //Enemy chooses a random pokemon
            ...
            enemyPokemon.remove(cPokemon); //Removes the pokemon for the list of pokemons to choose from
            pokeTools.clearScreen();
            ...
            while (enemySelected.isAlive()) {  //Checking if the enemy is still alive
                ...
                if (randNum.nextInt(2) == 1) {  //Player goes first
        ...
        pokeTools.clearScreen();  //Clears the screen for the next battle
    }
}
