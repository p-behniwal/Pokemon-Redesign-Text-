...
public AttackPokemon(Pokemon poke) { //"poke" the pokemon object
    name = poke.getName();
    hp = ohp = poke.getHp();
    type = poke.getType();
    ...
    public boolean playerAttack(AttackPokemon opponent, String username) {  //The attacking class for the player, this is in here because this interacts with the current pokemon a lot
        ...
        int moveNum, counter = 0; //The variables used for user input and the counter for selecting an attack
        ...
        while (true) {  //do not quit until an action has been selected
            ...
            if (moveNum == 0) {  //If the option selected is 0
                ...
                if (EC >= selectedAttack.getEc()) {  //Checks if the player has the required EC
...
public void Attack(Attack att, AttackPokemon opponent, String username) { //"att": attack used ; "opponent": who will be attacked ; "username": name of current player
    ...
    switch (att.getSpecial()) {  //Tests for what is the special of this attack
 ...
 */
public void hit(int damage, String type, AttackPokemon attacker) { //"damage": damage the attack will do ; "type": the type of special ; "attacker": the attacker
    if (attacker.getType().equals(weakness)) {  //If the attacker's type is the current pokemon's weakness, then double damage, super effective
        hp -= damage * 2; //Subtracts double damage
    ...
    if (type.equals("stun")) {  //Specials, stunning the current pokemon
        ...
        stunned = true;  //Sets the stunned attr to true
    } else if (type.equals("disable")) {
        pokeTools.delayPrintln(this.name + " is disabled.");
        disabled = true;
    }

    if (hp <= 0) {  //Checks if the pokemon has fainted
        fainted = true;
